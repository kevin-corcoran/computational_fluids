...
#include "definition.h"
  ...
  do i = gr_i0,gr_imax
     if (gr_xCoord(i) <= sim_shockLoc) then
        ...
        gr_V(DENS_VAR,i) = sim_densR ! + sim_ampl*sin(sim_freq*gr_xCoord(i))
        gr_V(VELX_VAR,i) = sim_velxR
        gr_V(PRES_VAR,i) = sim_presR
     end if
  ! for Blast2
  !    if (gr_xCoord(i) <= sim_shockLoc1) then
  !       gr_V(DENS_VAR,i) = sim_densL
  !       gr_V(VELX_VAR,i) = sim_velxL
  !       gr_V(PRES_VAR,i) = sim_presL
  !     elseif ( (gr_xCoord(i) > sim_shockLoc1) .and. (gr_xCoord(i) <= sim_shockLoc2)) then
