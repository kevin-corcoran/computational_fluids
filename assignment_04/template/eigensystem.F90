...
contains
  ...
  subroutine right_eigenvectors(V,conservative,reig)
    ...
    if (conservative) then
       ...
       ! reig(DENS_VAR,SHOCKLEFT) = -0.5*d/a
       ! reig(VELX_VAR,SHOCKLEFT) = 0.5
       ! reig(PRES_VAR,SHOCKLEFT) = -0.5*d*a
       ! ! reig(:,SHOCKLEFT) = -hdai*reig(:,SHOCKLEFT)
       ...
       ! reig(PRES_VAR,CTENTROPY) = 0.
       ! 
       ! reig(DENS_VAR,SHOCKRGHT) = 0.5*d/a
       ! reig(VELX_VAR,SHOCKRGHT) = 0.5
       ! reig(PRES_VAR,SHOCKRGHT) = 0.5*d*a
    endif
  ...
  subroutine left_eigenvectors(V,conservative,leig)
    ...
    if (conservative) then
       ...
       ! leig(DENS_VAR,SHOCKLEFT) = ekin + 2*a*u/g
       ! leig(VELX_VAR,SHOCKLEFT) = hda*ekin - 2*a*d/g
       ! leig(PRES_VAR,SHOCKLEFT) = -ekin + 2*a*u/g
       ! leig(:,SHOCKLEFT) = -hdai*leig(:,SHOCKLEFT)
