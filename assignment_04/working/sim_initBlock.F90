...
#include "definition.h"
  ...
  integer :: i
  real :: ekin, eint, x, del, gamm, dens
  ...
  do i = gr_i0,gr_imax
     ...
     elseif (sim_icType == 'isen_wave') then
        x = gr_xCoord(i) - 0.5
        gamm = .5*(sim_gamma-1.)
        del = 0.1
     ...
     elseif (sim_icType == 'sin') then
        x = gr_xCoord(i)*2.*PI
        gr_V(DENS_VAR,i) = 1.5 - 0.5*SIN(x)
        gr_V(VELX_VAR,i) = 1.
        gr_V(PRES_VAR,i) = 1./sim_gamma

     elseif (sim_icType == 'einfeldt') then
        if (gr_xCoord(i) < sim_shockLoc) then
           gr_V(DENS_VAR,i) = sim_densL
           gr_V(VELX_VAR,i) = sim_velxL
        ...
        gr_V(PRES_VAR,i) = (sim_gamma - 1.)*(gr_U(ENER_VAR,i) - 0.5*(gr_V(DENS_VAR,i)*gr_V(VELX_VAR,i)*gr_V(VELX_VAR,i)))
