...
#include "definition.h"
  ...
  real, dimension(2*gr_radius+1) :: G, un, WL, WR
  ...
  do i = gr_ibeg-1,gr_iend+1
     ...
     do var = DENS_VAR, PRES_VAR
        ...
        do s = 1, N
           WL(s) = -gr_dx*SUM(G(s:N))
           WR(s) =  gr_dx*SUM(G(1:s))
        end do
        !if (var == 1) print *, i, G
...
!!$        end do
        ...
        vecL(var) = dot_product(gr_GPZ(1, 1:N),WL)
        vecR(var) = dot_product(gr_GPZ(2, 1:N),WR)
